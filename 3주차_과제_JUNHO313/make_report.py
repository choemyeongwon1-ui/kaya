...
<h3>조회에서 걸린 것</h3>
<ul class="note">
<li>처음에는 개인 OC 가 IP 미등록으로 '사용자 정보 검증에 실패' 응답(HTTP 200)을 받아 공개 데모 키 <code>OC=test</code> 로 1차 조회했다.
마이페이지에 실습 PC IP 를 등록한 뒤 <b>개인 OC 로 43개 조문을 전부 재조회</b>했고, 다시 만든 CSV 두 개는 1차 조회본과 바이트 단위로 같았다. 키는 환경변수 LAW_OC 로만 두고 저장소에 적지 않았다.</li>
